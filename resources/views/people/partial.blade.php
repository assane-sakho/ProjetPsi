    ...
    <tbody>
      ...
      <tr>
        ...
        <td class="not-export-col">
          <button class="btn btn-warning" data-toggle="modal" data-target="#editModal" data-id="{{ $person->id }}" data-lastname="{{ $person->lastname }}" data-firstname="{{ $person->firstname }}" data-email="{{ $person->email }}" data-num="{{ $person->num }}" data-directoryId="{{ $person->directory->id}}" data-statusId="{{ $person->status->id}}">Modifier</button>
        </td>
        <td class="not-export-col">
          <button class="btn btn-danger" data-toggle="modal" data-target="#deleteModal" data-id="{{ $person->id }}" data-lastname="{{ $person->lastname }}" data-firstname="{{ $person->firstname }}" data-email="{{ $person->email ?? 'Non renseigné' }}" data-num="{{ $person->num }}" data-directory="{{ $person->directory->name}}" data-status="{{ $person->status->title}}">Supprimer</button>
        </td>
      </tr>
      @endforeach
    </tbody>
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <div id="importInputDiv">
          <label for="">Fichier : </label><input type="file" id="fileImport" name="fileImport" accept=".xlsx" onclick="$(this).val('')" />
        </div>
        </p>
        <div class="alert alert-danger" role="alert">
          <h3>Attention</h3><br />
          Veuillez vous assurer que le fichier est en .xlsx et qu'il comporte les colonnes <span class="text-info">NOM</span>, <span class="text-info">PRENOM</span>, <span class="text-info">EMAIL</span>, <span class="text-info">NUMERO</span>, <span class="text-info">ANNUAIRE</span>, et <span class="text-info">STATUT</span> (tout en majuscule).</p>
          </p>
...
<script>
  ...
  function fillJsonObjArray(jsonData) {
    var jsonObj = [];
  ...
  function initDatatable(jsonObj) {
    $("#tableImport").DataTable().clear().draw().destroy();
    return $('#tableImport').DataTable({
      data: jsonObj,
      columns: [{
          data: 'canBeAdded'
        },
    ...
    table.rows().every(function(rowIdx) {
      ...
      if (i == 0) {
        checkIfExistPromise(d["lastname"], d["firstname"], d["num"])
          .then(exist => {
            var cellCanBeAdded = table.cell({
              row: rowIdx,
              column: 0
            });
            var textExist = exist == "true" ? "Oui" : "Non";
            var canBeAdded = exist == "true" ? false : true;
            table.cell({
              row: rowIdx,
              column: 7
            }).data(textExist);
    ...
    $.each(data, function(key, value) {
      ...
      if (value.canBeAdded == '<i class="fa fa-check text-success"></i>') {
        ...
        var data =
          ...
          "&statusTitle=" + status;
        $.post("/People/AddFromImport", data);
      }
    });
